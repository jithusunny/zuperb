...
async def home(request: Request):
    return templates.TemplateResponse("home.html", {"request": request, "visitor_name": request.state.visitor_name, "theme": request.state.theme})

@app.get("/updates", response_class=HTMLResponse)
async def updates(request: Request):
    return templates.TemplateResponse("updates.html", {"request": request, "updates": CHANGES, "visitor_name": request.state.visitor_name, "theme": request.state.theme})

@app.get("/stats", response_class=HTMLResponse)
async def stats(request: Request, db=Depends(get_db), page: int = 1):
    # Fetch paginated logs
    logs_data = paginate(db.query(VisitLog).order_by(VisitLog.timestamp.desc()), page, per_page=10)
...
async def recipes(request: Request):
    return templates.TemplateResponse("recipes.html", {"request": request, "recipes": RECIPES, "visitor_name": request.state.visitor_name, "theme": request.state.theme})

@app.get("/quotes", response_class=HTMLResponse)
async def quotes(request: Request):
    random_quote = random.choice(QUOTES)
    return templates.TemplateResponse("quotes.html", {"request": request, "quote": random_quote, "visitor_name": request.state.visitor_name, "theme": request.state.theme})
...
async def about(request: Request):
    return templates.TemplateResponse("about.html", {"request": request, "visitor_name": request.state.visitor_name, "theme": request.state.theme})
